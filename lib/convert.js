                ...
                if (tempParamKey === null) {
                    ...
                    if (change == "#") {
                        ...
                        if (isObject(tempParamKey) || isArray(tempParamKey)) {
                            ...
                            //myyrakle;
                            if (typeof tempParamKey === "number") {
                                tempParamKey = String(tempParamKey);
                                convertString = convertString.replace(
                                    reg,
                                    tempParamKey
                                );
                            } else if (typeof tempParamKey === "boolean") {
                                tempParamKey = String(tempParamKey);
                                convertString = convertString.replace(
                                    reg,
                                    tempParamKey
...
var convertChoose = function (children, param, namespace, myBatisMapper) {
    try {
        for (
            var i = 0, whenChildren;
            (whenChildren = children.children[i]);
            i++
        ) {
            if (
...
var convertTrimWhere = function (children, param, namespace, myBatisMapper) {
    ...
    try {
        switch (children.name.toLowerCase()) {
            ...
            default:
                throw new Error(
                    "Error occurred during convert <trim/where> element."
                );
                break;
        }
        ...
        for (
            var j = 0, nextChildren;
            (nextChildren = children.children[j]);
            j++
        ) {
            convertString += convertChildren(
                nextChildren,
...
var convertInclude = function (children, param, namespace, myBatisMapper) {
    ...
    try {
        var refid = convertParametersInner(
            "#",
            children["attrs"]["refid"],
            param
        );
